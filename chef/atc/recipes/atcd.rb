...
node.network.interfaces.each do |name, eth|
    if eth.encapsulation == "Ethernet"
        node.default['sysctl']['params']['net']['ipv4']['conf'][name]['arp_ignore'] = 1
        node.default['sysctl']['params']['net']['ipv4']['conf'][name]['arp_announce'] = 2
    end
end
...
if node.vagrant?
    ...
    node.network.interfaces.each do |name,eth|
        lan_iface = name if (eth.encapsulation == "Ethernet") and (name != wan_iface) 
    end

    if lan_iface == ''
        log "You only have 1 network interface. ATCD requires 2 network interfaces. Chef will likely fail."
        lan_iface = 'eth1'
    end
    ...
    # DHCP
    # By providing IP on the lan interface, we can get Genymotion instances to route
    # through atc
    require 'ipaddr'
    node.default['dhcp']['interfaces'] = [lan_iface]
    include_recipe 'dhcp::server'

    ipv4 = node['network']['interfaces'][lan_iface]['addresses'].select do |k, v|
        v.family == 'inet'
    end
    ip = ipv4.keys[0]
